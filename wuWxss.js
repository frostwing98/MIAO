...
function doWxss(dir, cb, mainDir, nowDir) {
    ...
    function cssRebuild(data) {//need to bind this as {cssFile:__name__} before call
        ...
        function statistic(data) {
            function addStat(id) {
                if (global.filetype==='page-frame.html'){
                    if (!importCnt[id]) importCnt[id] = 1, statistic(pureData[id]);
				    else ++importCnt[id];
                }else if (global.filetype==='app-wxss.js'){
                    if(!importCnt[id]){
                        if(pureData){
                            importCnt[id]=1
    ...
    function tryrun(flag,dir,frameFile, mainCode, files){
        global.filetype=flag
    ...
    wu.scanDirByExt(dir, ".html", files => {
        ...
        wu.get(frameFile, code => {
            ...
            console.log("filename: "+frameFile)
            pureData=""
            
            
            try{
                pureData = vm.run(code + "\n_C");
                tryrun('page-frame.html',dir,frameFile,mainCode,files)
            }catch(error){
                try{
                    pureData = vm.run(code + "}");
                    tryrun('app-wxss.js',dir,frameFile,mainCode,files)
                }catch(error){
                    console.log(error)
                }
            }
